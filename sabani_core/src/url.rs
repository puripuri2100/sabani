...
mod tests {
  ...
  fn test_url_host_port_path_searchquery() {
    let url = "http://example.com:8888/index.html?a=123&b456".to_string();
    let expected = Ok(Url {
      url: url.clone(),
      host: "example.com".to_string(),
      port: "8888".to_string(),
      path: "index.html".to_string(),
      searchpart: "a=123&b456".to_string(),
    });
    assert_eq!(expected, Url::new(url).parse())
  }
  ...
  #[test]
  fn test_no_scheme() {
    let url = "example.com:8888/index.html?a=123&b456".to_string();
  ...
  fn test_unsupported_scheme() {
    let url = "https://example.com:8888/index.html?a=123&b456".to_string();
    let expected = Err("Only HTTP scheme is supported.".to_string());
    assert_eq!(expected, Url::new(url).parse())
  }
}
